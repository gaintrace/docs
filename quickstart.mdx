...
<Steps>
  <Step title="Get an API key">
    ...
    scopes you need (for example `read:accounts`, `write:events`). Copy the key —
    it's shown once.
  </Step>
  <Step title="Read your accounts">
    Call the accounts endpoint with your key as a Bearer token.

    <CodeGroup>

    ```bash cURL
    curl "https://app.gaintrace.com/api/v1/accounts?limit=5" \
      -H "Authorization: Bearer gt_live_your_key_here"
    ```

    ```javascript Node
    const res = await fetch("https://app.gaintrace.com/api/v1/accounts?limit=5", {
      headers: { Authorization: "Bearer gt_live_your_key_here" },
    });
    const { data, meta } = await res.json();
    console.log(meta.count, "accounts");
    ```
    ...
    res = requests.get(
        "https://app.gaintrace.com/api/v1/accounts",
        headers={"Authorization": "Bearer gt_live_your_key_here"},
        params={"limit": 5},
    )
    print(res.json()["meta"]["count"], "accounts")
    ```
