...
<CardGroup cols={3}>
  <Card title="Tier C — workflows" icon="wand-magic-sparkles">
    One-shot answers to CS-shaped questions. Prefer these first.
  </Card>
  <Card title="Tier B — namespaced" icon="layer-group">
...
- **Scopes** — each tool requires a scope (for example `read:accounts`, `write:signals`). Your OAuth grant or API key decides which tools are available. Read tools need a `read:` scope; writes need the matching `write:` scope.
