...
| Event | Fires when |
| --- | --- |
| `account.created` | An account is created via the API |
| `account.updated` | An account is updated or upserted |
| `contact.created` | A contact is created |
| `contact.updated` | A contact is updated |
| `contact.deleted` | A contact is deleted |
| `signal.created` | A signal is recorded against an account |
...
curl -X POST https://app.gaintrace.com/api/v1/webhooks \
  ...
  -d '{
    ...
    "events": ["signal.created", "account.updated"],
